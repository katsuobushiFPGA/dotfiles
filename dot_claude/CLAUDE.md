...
実装系のタスク（新機能追加・バグ修正・リファクタ）は **テストファースト** で進める。`/tdd` スキル（red-green-refactor）に沿うのが基本。

**基本サイクル**:
1. 🔴 **Red**: まず失敗するテストを書く（バグ修正なら再現テスト、新機能なら期待する振る舞いのテスト）
2. 🟢 **Green**: テストを通す最小限の実装を書く
3. 🔵 **Refactor**: テストが緑のまま、重複排除・命名改善・構造整理をする
4. 次の振る舞いに進む（1 に戻る）

**新規プロジェクト／新規モジュールの場合**:
- テストフレームワークが入っていなければ **最初にテスト基盤を整える**（言語標準のもの優先: Vitest/Jest, pytest, go test, RSpec, cargo test, bats など）
- `npm test` / `pytest` / `go test ./...` 相当のワンコマンドで全テストが走る状態にしてから 1 件目のテストを書く
- CI が無ければ最低限ローカルで回せる状態でよい（CI 整備は別タスク扱い）

**対象外**:
- テストが書けないもの（chezmoi テンプレ、shell の対話設定、CLAUDE.md などのドキュメント、IaC の宣言的設定）
- 既存コードに後付けでテスト基盤を入れるのが過剰なワンショット作業（PR 説明・コミットメッセージで理由を残す）
- UI の見た目調整・微細なスタイル変更（動作確認で代替）

**Why**: テストを後付けすると「通るように書いたテスト」になってバグ検出力が落ちる。先に Red を作ることで仕様を言語化でき、Green で過剰実装を避けられる。新規プロジェクトでテスト基盤を後回しにすると、結局最後まで書かれずに負債化するので最初に入れる。

**運用**:
- 実装の方針が固まったら、コード書く前に `/tdd` を起動するか、最低でも「これから書くテストはこれ」と宣言してからテストファイルに着手する
- Green になるまで本実装に余計な機能を足さない（YAGNI）
- Refactor 後は必ずテスト再実行して緑を確認
