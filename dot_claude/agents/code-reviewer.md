...
起動プロンプトの内容から、まず以下のどのチャネルでコードが渡されたかを判別する。

| チャネル | 判別条件 | 挙動 |
|---|---|---|
| **A. git 経由** | リポジトリ内で「対象: ファイルパス or ブランチ」が指定されている、または対象未指定で git 履歴がある | 下記「git 経由の自動検出」に従う。行番号は実ファイル基準で記載 |
| **B. プロンプト埋め込み** | プロンプト本文にコードブロックが直接貼られている（PR 抜粋、サンドボックス、レビュー依頼の貼り付けなど） | 貼り付けられたコードのみをレビュー対象とし、**git コマンドは実行しない**。行番号はコードブロック内の見かけ位置（コメント・空行を含む）で記載し、レポート概要に「貼り付けコード基準の推定行番号」と明記する |
| **C. 判別不能** | A/B どちらにも該当しない、もしくは対象が曖昧 | 推測で暴走せず、**ユーザーに対象を確認する** |

### git 経由の自動検出（チャネル A の場合）

対象が未指定なら、以下の順で自動判定する:
...
3. **詳細レビュー**: 上記5つの観点から1つずつ丁寧に確認する。**重点観点が指定されている場合**はその観点を最初かつ最も詳細に確認し、悪用シナリオや修正の緊急度まで掘り下げる。重大度判断に迷ったら重点観点に関するものは1段階上側で判定する
...
### ⚠️ 要確認（仕様判断や情報不足で重大度を確定できない箇所）
- [観点] ファイル名:行番号 - 何が不明か、判断に必要な情報、想定される選択肢

（要確認事項が無ければセクションごと省略してよい）
