...
- **フェーズ未指定**: 対象の性質からフェーズを推定する。推定基準は以下:
  - 要件定義書・PRD・RFC（未着手）→ **要件**
  - 設計書・アーキ図・dataLayer 設計・タグマップ → **設計**
  - コード・PR・タグ設定 JSON・設定ファイル → **実装**
  - 本番 URL・稼働中のダッシュボード・運用手順書 → **運用**
  - 判別できない場合はレポート冒頭で「フェーズを○○と推定した」と明示し、誤っていれば再指定を促す。
...
フェーズごとに「何を最重要に見るか」「何は後工程に送ってよいか」を変える。指定フェーズの **最重点観点を優先**し、隣接フェーズは参考指摘に留める。

| フェーズ | 最重点（🔴 昇格候補） | 次点（🟡 中心） | 参考（🟢 軽め） |
|---|---|---|---|
| 要件 | KPI/ファネル未定義、CMP要件欠落、広告媒体連携要件の抜け、法域対応の前提 | 業態固有 KPI の妥当性、CDP/MA 導入是非、計測スタック選定の前提 | 具体タグ設計、実装詳細、CWV 数値 |
| 設計 | dataLayer スキーマ破綻、Consent Mode v2 未組込、GTM/sGTM 構成の欠陥、イベント命名の非一貫性 | CDP/MA 連携のデータフロー、A/B テスト基盤、同意前送信イベントの切り分け | KPI そのものの妥当性（要件に差戻さない限り）、SEO 本文最適化 |
| 実装 | dataLayer 実装バグ、Consent 制御漏れ、UTM/Redirect 破壊、SEO 技術要件（canonical/robots/sitemap）、個人情報の誤送信 | CWV 悪化、構造化データ不備、タグ発火条件の誤り、ページ別イベント欠落 | 設計段階の再検討、KPI 定義見直し |
| 運用 | 計測データ欠損・異常値放置、ITP/3rd party cookie 影響の未対処、CAPI 未設定での EMQ 低下、同意率の継続低下 | SEO 順位低下の原因分析、メール到達率、ダッシュボード運用 | 設計／実装のリファクタ提案（影響大なら昇格） |

**重み付けの運用ルール:**

- 指定フェーズで「最重点」に該当する問題は、原則 🔴 として扱う。
...
- **同一観点が複数フェーズの最重点に重複している場合**（例: Consent Mode v2 は Phase B「設計」と Phase C「実装」の両方で最重点）、**現に観測したフェーズの最重点として判定する**。例: 実装段階のコードを見て Consent Mode v2 未組込が残っていたら、Phase C-3「Consent 制御漏れ」として 🔴 と扱う（Phase B の論点として注記し直す必要はない）。
...
| 絵文字 | 基準 |
|---|---|
| 🔴 | 法規制違反リスク（GDPR, 個人情報保護法）、計測全滅・本番事故、Core Web Vitals 致命的、プライバシー違反（CMP 未実装でタグ発火、個人情報の意図せぬ送信など） |
...
**セクションの出力ルール:**
- 「概要」と「🎯 次のアクション提案」は**常に出力**する。
- 「🔴／🟡／🟢／⚠️／✅」の各セクションは、**該当する内容がある場合のみ見出しごと出力**する（空セクションは見出しごと省略してノイズを避ける。"該当なし" と書かない）。
- 「📊 ファネル & KPI マッピング」は、**要件フェーズ指定時は必須**、それ以外のフェーズでは「KPI/ファネル設計の提案がレビュー結果から自然に出てくる場合」のみ任意で出力。
...
### 概要 （常に出力）
- 対象: <ファイル・URL>
- フェーズ: <要件／設計／実装／運用。複数なら「要件, 設計」のように列挙>
- 業態: <BtoC EC 等>
- 重点観点: <全観点 or 指定観点>
- 所感: 全体評価を1〜2行

### 🔴 重大な問題（必ず対応推奨） （該当なしなら見出しごと省略）
- [フェーズ/観点] 該当箇所 - 問題と対応案
  - 根拠: 一次情報の URL や引用
...
### ✅ 良い点 （本当に優れたものがあれば。なければ見出しごと省略）
- 特筆すべき設計・実装

### 📊 ファネル & KPI マッピング （要件フェーズ時必須／他フェーズでは提案できる場合のみ任意）
