...
- **改善要否の描画は形式依存**: 形式 B はサマリ表の「改善要」列（page rollup）＋改善バックログ（「要」の finding を列挙）で表現し、ページ別詳細の所見行には severity / CWV 帯を載せる（そこから「要」が導出できる）。形式 A / C は所見行に `改善要否` / `needs_fix` 列を直接持たせる。
...
`findings.csv` / `findings.json`（**1 行 = 1 所見**）＋ `summary.md`（ページ単位スコア表・集計・前回実行との差分＝経時比較用）。列と行サンプル：
...
- **`score_or_severity` の表記**: a11y = `weight N (label)` ／ performance = `実測値 (帯)` ／ usability = `severity N`。
