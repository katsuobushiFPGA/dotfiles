...
- **リダイレクト追従時の URL 表記**: 監査対象 URL が 30x で別 URL にリダイレクトしても Lighthouse は最終ページを監査する（例: `/psf/` → `/psf-landing/`）。レポートには**要求した URL を主表記し、最終 URL が異なるときだけ括弧で併記**する（`/psf/ → /psf-landing/`）。要求 URL が一意のラベルなので per-page の見出し・サマリ行は要求 URL で揃える。
